...
void setup() {
  ...
  root_display = SD.open(DIRECTORY);
  if (!root_display || !root_display.isDirectory()) {
...
void select_next_file() {
  ...
  // move to next file
  current_file = root_display.openNextFile();

  // If we reach the end of the folder, rewind
  if (!current_file) {
    root_display.rewindDirectory();
    current_file = root_display.openNextFile();
  }
...
void list_dir(fs::FS &fs, const char * dirname) {
  ...
  File root_display = fs.open(dirname);
  if (!root_display || !root_display.isDirectory()) {
    Serial.println("Failed to open directory");
    return;
  }

  File file = root_display.openNextFile();
  while (file) {
    if (file.isDirectory()) 
    {
      Serial.print("DIR : ");
    ...
    file = root_display.openNextFile();
  }
}
